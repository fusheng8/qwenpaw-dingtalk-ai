      ...
      h(Spin, {spinning: busy},
        ...
        h(Form, {form, layout: "vertical", onFinish: save, initialValues: {enabled: false, require_mention: true, retention_days: 30}},
          h(Card, {title: "2. 凭据与 AI 卡片", style: {marginBottom: 20}},
            ...
            h(Divider),
            h(T.Paragraph, null, "下载模板，在钉钉卡片平台选择消息卡片并启用 AI 卡片，再导入 JSON，保存发布后复制模板 ID。"),
            ...
            h(T.Paragraph, {type: "secondary"}, "审批使用另一个吊顶卡片模板，导入发布后填写下方 ID。"),
            h(Button, {onClick: () => download(true), style: {marginBottom: 16}}, "下载审批吊顶模板"),
            item("top_template_id", "审批吊顶模板 ID", h(Input, {placeholder: "选择吊顶卡片，导入并发布后填写"}), {extra: "待审批时在会话顶部显示，处理后自动关闭。未配置时审批不会自动通过。"}),
