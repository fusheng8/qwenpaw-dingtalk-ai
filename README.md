...
```sh
qwenpaw plugin install https://github.com/fusheng8/qwenpaw-dingtalk-ai/releases/download/v1.0.25/dingtalk-ai-1.0.25.zip --force
```
...
1. [下载卡片导入文件](https://github.com/fusheng8/qwenpaw-dingtalk-ai/releases/download/v1.0.25/dingtalk-ai-card.json)，也可在插件设置页点击「下载 AI 卡片模板」。
...
4. 另行[下载审批吊顶模板](https://github.com/fusheng8/qwenpaw-dingtalk-ai/releases/download/v1.0.25/dingtalk-approval-top-card.json)，在卡片平台新建卡片，卡片类型选择**吊顶卡片**，导入、发布；将 ID 填入「审批吊顶模板 ID」。两个模板 ID 分别填写，不能混用。启用渠道并保存，Robot Code 通常留空即可。
...
1.0.25 图片改为原生图片消息：公网图片直接发送，本地和 Base64 图片上传后使用 sampleImageMsg/photoURL=mediaId，不再包装 Markdown 图片卡片。上传或发送失败明确报错；其他附件处理不变。无需重新导入模板。依据：[钉钉机器人消息类型](https://open.dingtalk.com/document/development/robot-message-type)。

1.0.24 将过程中的工具行简化为「执行命令 xxx」「读取 xxx」「编辑 xxx」「搜索 xxx」「调用 xxx」，执行状态只在点击后的详情面板显示，完整参数与结果保持不变。升级插件即可生效，无需重新导入模板。

1.0.23 修复图片、文件、视频、音频输出被忽略的问题，覆盖直接发送、完成事件、流式结束事件及 send_file_to_user 工具输出。升级插件即可，无需重新导入模板。多媒体真实上传及客户端播放仍需钉钉环境验收。

1.0.22 将插件默认分段阈值及可配置上限统一调整为 3,000 UTF-8 字节。长回答首屏、阅读全文和工具详情使用同一阈值；思考短预览与审批摘要布局不变。显式配置的较小 page_bytes 仍保留。需要升级插件，无需重新导入模板。
...
- 长回答和工具结果受钉钉卡片大小限制，默认每页最多 3,000 UTF-8 字节（保守解释官方流式内容 3 K 建议，并非整张卡片或普通参数的官方硬上限）；长回答首屏显示第一页并提供「阅读全文」。详情不会因预览长度被丢弃。
...
支持文本、模型 reasoning、结构化工具事件、工具审批及图片/文件/视频/音频输出。文字保持单张 AI 卡片，附件复用官方渠道上传并以独立消息发送；会话通道失败时回退 Open API，失败会报错而非静默丢弃。同轮已成功发送的相同附件去重。输入多媒体继续沿用官方渠道能力。不提供定时任务主动文本推送。音频通过文件形式投递，视频在会话通道尝试视频消息，缺少封面或不支持格式时回退文件；Open API 通道可能将音视频作为文件发送，不保证所有媒体原生气泡播放。

已通过 93 项自动化测试，覆盖真实 QwenPaw 2.2.1 插件加载与流事件分发、对话生命周期、原生折叠状态、结果无损分页、过程详情私有投放、审批隔离与重复点击、重启恢复和模板结构。已在隔离的真实千问派控制台验证图形配置页面、表单校验与保存，并通过官方 CLI 的 URL ZIP 安装。**卡片平台导入发布、手机/桌面客户端渲染、组织扫码权限及真实审批闭环仍需实际钉钉账号验收；代码和结构测试不能替代该验收。**
